...
def get_track(songhash, track):
    try:
        track_path = os.path.join(TRACK_STORAGE, f"{songhash}_{track}.mp3")

        if not os.path.exists(track_path):
            return jsonify({"error": "Track not found"}), 404

        return send_file(track_path, as_attachment=True, attachment_filename=f"{track}.mp3")
    except Exception as e:
...
def remove_track(songhash, track):
    try:
        ...
        if os.path.exists(track_path):
            ...
            return jsonify({"status": "Track removed"}), 200
        else:
            return jsonify({"error": "Track not found"}), 404
    except Exception as e:
